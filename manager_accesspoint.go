...
import (
	...
	"github.com/sofiworker/gonm/libs/utils"
...
func getApSecType(ap nmdbus.AccessPoint) (apSecType, error) {
	...
	if err != nil {
		logger.SDebugf("get flags failed, err: %v", err)
		return apSecNone, err
	}
	wpaFlags, err := ap.WpaFlags().Get(0)
	if err != nil {
		logger.SDebugf("get wpa flags failed, err: %v", err)
		return apSecNone, err
	}
	rsnFlags, err := ap.RsnFlags().Get(0)
	if err != nil {
		logger.SDebugf("get rsn flags failed, err: %v", err)
		return apSecNone, err
	}
...
func (m *Manager) isHidden(ssid string) bool {
	...
	for _, conn := range wirelessCon {
		if conn.Ssid == ssid && conn.Hidden {
			logger.SDebugf("access point %s is hidden ", ssid)
			return true
		}
	}
	return false
...
func (m *Manager) activateAccessPoint(uuid string, apPath, devPath dbus.ObjectPath) (cpath dbus.ObjectPath, err error) {
	logger.SDebugf("ActivateAccessPoint: uuid=%s, apPath=%s, devPath=%s", uuid, apPath, devPath)
	...
	if err != nil {
		...
		logger.SDebugf("active access point sec %v", secType)
	}
	if uuid != "" {
