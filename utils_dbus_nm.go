...
func nmDestroyDevice(dev nmdbus.Device) {
	if dev == nil {
		logger.SError("Device to destroy is nil")
		return
	}
...
func nmDestroyAccessPoint(ap nmdbus.AccessPoint) {
	if ap == nil {
		logger.SError("AccessPoint to destroy is nil")
		return
	}
...
func nmDestroySettingsConnection(conn nmdbus.ConnectionSettings) {
	if conn == nil {
		logger.SError("SettingsConnection to destroy is nil")
		return
	}
...
func nmAddAndActivateConnection(data connectionData, devPath dbus.ObjectPath, forced bool) (cpath, apath dbus.ObjectPath, err error) {
	...
	if err != nil {
		...
		logger.SError(err, "devPath:", devPath)
		return
	}
	return
}
...
func nmGetConnectionId(cpath dbus.ObjectPath) (id string) {
	...
	if len(id) == 0 {
		logger.SError("get Id of connection failed, id is empty")
	}
	return
}
...
func nmGetConnectionList() (connections []dbus.ObjectPath) {
	...
	for {
		...
		if err != nil {
			...
			logger.SDebugf("get connection list failed, retry to get, err: %v", err)
			retry++
			time.Sleep(2 * time.Second)
			continue
		}
		// get connection success
		logger.SDebugf("get all connection success, %v", connections)
		break
	}
	return
}
