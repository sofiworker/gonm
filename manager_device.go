...
import (
	...
	mmdbus "github.com/sofiworker/gonm/system/org.freedesktop.modemmanager1"
...
func (m *Manager) adjustDeviceStatus() {
	for _, path := range nmGetDevices() {
		...
		if stat == nm.NM_DEVICE_STATE_NEED_AUTH {
			...
			if err != nil {
				logger.Warn("failed to enable device:", err)
			}
		}
	}
}
...
func (m *Manager) newDevice(devPath dbus.ObjectPath) (dev *device, err error) {
	...
	case nm.NM_DEVICE_TYPE_ETHERNET:
		...
		if nmHasSystemSettingsModifyPermission() {
			carrierChanged := func(hasValue, value bool) {
				...
				logger.SInfof("wired plugin%s", dev.Path)
			...
			if err != nil {
				logger.SWarnf("failed to monitor Wired-Carrier's change:%v", err)
			}
	...
	_, err = dev.nmDev.Device().ConnectStateChanged(func(newState uint32, oldState uint32, reason uint32) {
		logger.SDebugf("device %s state changed, %d => %d, reason[%d] %s",
	...
	if err != nil {
		...
		logger.SDebugf("get interface-flags success, flags: %v", dev.InterfaceFlags)
	}
	...
	err = dev.nmDev.Device().InterfaceFlags().ConnectChanged(func(hasValue bool, value uint32) {
		...
		logger.SDebugf("interface-flags changed, flags: %v", value)
...
func (m *Manager) doRemoveDevice(devs []*device, i int) []*device {
	logger.SInfof("remove device %#v", devs[i])
...
func (m *Manager) enableDevice(devPath dbus.ObjectPath, enabled bool, activate bool) (err error) {
	...
	logger.SDebugf("dev %v, enabled: %v, activate: %v", devPath, enabled, activate)
	...
	if enabled {
		if cpath != "/" && activate {
			...
			logger.SDebugf("active connection success, dev: %v, uuid: %v", devPath, uuid)
		}
		...
		logger.SDebugf("begin to set device auto-connect state, dev: %v", devPath)
		nmDev, err := nmNewDevice(devPath)
		if err != nil {
		...
		logger.SDebugf("device set auto-connect success, dev: %v", devPath)
	}
...
func (m *Manager) checkGatewayConnectivity(ipPath dbus.ObjectPath) bool {
	...
	logger.SDebugf("The active connection ip4 info: %v",
		ip4Info)
