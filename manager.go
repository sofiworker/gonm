...
import (
	...
	airplanemode "github.com/sofiworker/gonm/system/org.deepin.dde.airplanemode1"
	ipwatch "github.com/sofiworker/gonm/system/org.deepin.dde.ipwatch1"
	sysNetwork "github.com/sofiworker/gonm/system/org.deepin.dde.network1"
	sessionmanager "github.com/sofiworker/gonm/system/org.deepin.dde.sessionmanager1"
	...
	secrets "github.com/sofiworker/gonm/system/session/org.freedesktop.secrets"
)
...
func (m *Manager) init() {
	logger.SInfo("initialize network")
	...
	if err != nil {
		logger.SErrorf("Failed to connect self session:%v", err)
	}
	...
	logger.SDebugf("current session activated state: %v", globalSessionActive)
	...
	err = nmManager.PropState().ConnectChanged(func(hasValue bool, value uint32) {
		...
		logger.SDebugf("current primary typ is %v, prop changed: %v need to reactive vpn", typ, value)
		// get delay vpn state
	...
	m.syncConfig = dsync.NewConfig("network", &syncConfig{m: m},
		m.sessionSigLoop, dbusPath)
}

func (m *Manager) destroy() {
	logger.SInfo("destroy network")
	m.multiVpn = nil
	...
	_, err := dbusDaemon.ConnectNameOwnerChanged(func(name, oldOwner, newOwner string) {
		if name == "org.freedesktop.NetworkManager" {
			...
			if len(newOwner) != 0 {
				...
				logger.SInfo("network-manager is starting")
				time.Sleep(1 * time.Second)
				m.init()
			} else {
				// network-manager stopped
				logger.SInfo("network-manager stopped")
				m.destroy()
			}
		}
	})
...
func (m *Manager) doPortalAuthentication() {
	...
	logger.SDebugf("portal addr is %v", portal)
...
func (m *Manager) autoConnectVpn() {
	...
	logger.SDebugf("all auto connect vpn is %v", uuidList)
