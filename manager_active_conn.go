...
func (m *Manager) initActiveConnectionManage() {
	...
	sysSigLoop.AddHandler(&dbusutil.SignalRule{
		...
		if strings.HasPrefix(string(sig.Path),
			...
			if ok {
				...
				if ok {
					logger.SDebugf("active connection %s SpecificPath changed %v", sig.Path, specificPath)
			...
			if ok {
				...
				if stateChanged {
					logger.SDebugf("active connection %s state changed %v", sig.Path, state)
	...
	sysSigLoop.AddHandler(&dbusutil.SignalRule{
		...
		if strings.HasPrefix(string(sig.Path),
			...
			logger.SDebug(sig.Path, "vpn state changed", state, reason)
