...
func doGetSettingKey(data connectionData, section, key string) (value interface{}) {
	...
	if !ok {
		logger.SErrorf("invalid section: data[%s]", section)
		return
	}
	...
	if isInterfaceNil(value) {
		...
		// message
		logger.SErrorf("getSettingKey: data[%s][%s] is nil", section, key)
	}
...
func setSettingKey(data connectionData, section, key string, value interface{}) {
	...
	sectionData, ok := data[section]
	if !ok {
		logger.SErrorf(`set connection data failed, section "%s" is not exits yet`, section)
		return
	}
...
func removeSettingKey(data connectionData, section string, keys ...string) {
	logger.SDebugf("removeSettingKey data[%s], %s", section, keys)
	sectionData, ok := data[section]
	if !ok {
		return
	}
